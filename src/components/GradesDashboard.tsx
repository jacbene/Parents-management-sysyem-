...
export default function GradesDashboard({
  ...
  const handleExportTrendPDF = () => {
    ...
    const drawPageHeaderFooter = (num: number) => {
      ...
      doc.setTextColor(148, 163, 184); // Slate 400
      const transcriptLabel = isEn 
        ? `Academic Trend & Diagnostic Report - ${activeStudent.name} • Class: ${activeStudent.grade} ${activeStudent.classRoom}`
        : `Analyse de Tendance & Diagnostic Académique - ${activeStudent.name} • Classe : ${activeStudent.grade} ${activeStudent.classRoom}`;
      doc.text(transcriptLabel, margin, 9);
    ...
    // Title Block
    doc.setFillColor(15, 118, 110); // Teal 700 for trend/performance vibe
    doc.rect(margin, y, contentWidth, 14, 'F');
    ...
    doc.setTextColor(255, 255, 255);
    const mainTitle = isEn 
      ? "STUDENT ACADEMIC TREND & DIAGNOSTIC PERFORMANCE REPORT" 
      : "RAPPORT DE DIAGNOSTIC & TENDANCE ACADÉMIQUE DE L'ÉLÈVE";
    doc.text(mainTitle, margin + 5, y + 9);
    ...
    const dobLabel = isEn 
      ...
      : `Date de naissance : ${activeStudent.dob ? new Date(activeStudent.dob).toLocaleDateString('fr-FR') : 'Non renseignée'}`;
    const classLabel = isEn 
      ? `Class / Grade: ${activeStudent.grade} • Classroom: ${activeStudent.classRoom}` 
      : `Classe / Niveau : ${activeStudent.grade} • Salle : ${activeStudent.classRoom}`;
    doc.text(`${dobLabel}  |  ${classLabel}`, margin + 6, y + 14);

    const teacherLabel = isEn
      ? `Main Class Teacher: ${activeStudent.teacherName || 'Not assigned'} (${activeStudent.teacherEmail || 'No email'})`
      : `Professeur Principal : ${activeStudent.teacherName || 'Non assigné'} (${activeStudent.teacherEmail || 'Pas de courriel'})`;
    doc.text(teacherLabel, margin + 6, y + 19);
    ...
    const subjectAveragesList = Array.from(subjectAveragesMap.entries()).map(([subj, data]) => ({
      subject: subj,
      average: Number((data.total / data.count).toFixed(1)),
      count: data.count
    })).sort((a, b) => b.average - a.average);

    const strongest = subjectAveragesList.length > 0 ? subjectAveragesList[0] : null;
    const weakest = subjectAveragesList.length > 0 ? subjectAveragesList[subjectAveragesList.length - 1] : null;

    // Standard deviation (volatility)
    const varianceVal = totalTestsNum > 0 ? (grades.reduce((acc, g) => {
      const score20 = (g.score / g.maxScore) * 20;
      return acc + Math.pow(score20 - avgBase20Val, 2);
    }, 0) / totalTestsNum) : 0;
    const stdDev = Math.sqrt(varianceVal);

    // Trend comparison (halves)
    const chronologicalGrades = [...grades].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    const halfIndex = Math.floor(chronologicalGrades.length / 2);
    const firstHalfGrades = chronologicalGrades.slice(0, halfIndex);
    const secondHalfGrades = chronologicalGrades.slice(halfIndex);
    const firstHalfAvg = firstHalfGrades.length > 0 ? (firstHalfGrades.reduce((acc, g) => acc + (g.score / g.maxScore) * 20, 0) / firstHalfGrades.length) : 0;
    const secondHalfAvg = secondHalfGrades.length > 0 ? (secondHalfGrades.reduce((acc, g) => acc + (g.score / g.maxScore) * 20, 0) / secondHalfGrades.length) : 0;
    const trendDiff = secondHalfAvg - firstHalfAvg;

    const getMentionStr = (avg: number) => {
      if (avg >= 16) return isEn ? 'Highest Honors' : 'Très Bien';
      if (avg >= 14) return isEn ? 'High Honors' : 'Bien';
      if (avg >= 12) return isEn ? 'Honors' : 'Assez Bien';
      if (avg >= 10) return isEn ? 'Pass' : 'Passable';
      return isEn ? 'Needs Improvement' : 'Insuffisant';
    };

    // Draw KPI Grid: 4 boxes
    const cardW = (contentWidth - 9) / 4; // Approx 42mm each
    const cardH = 22;
    ...
    doc.text(isEn ? "GENERAL AVERAGE" : "MOYENNE GÉNÉRALE", margin + 3, y + 5);
    doc.setFontSize(13);
    doc.setTextColor(15, 23, 42);
    ...
    doc.text(isEn ? "CONSISTENCY (SD)" : "ÉCART-TYPE (RÉGULARITÉ)", margin + cardW + 6, y + 5);
    doc.setFontSize(13);
    doc.setTextColor(15, 23, 42);
    doc.text(`${stdDev.toFixed(2)}`, margin + cardW + 6, y + 13);
    doc.setFontSize(6.5);
    doc.setTextColor(100, 116, 139);
    const consistencyLabel = stdDev < 1.5 
      ? (isEn ? "Highly Regular" : "Très Régulier") 
      : stdDev < 3.0 
        ? (isEn ? "Moderately Regular" : "Modérément Régulier") 
        : (isEn ? "Highly Volatile" : "Forte Volatilité");
    doc.text(consistencyLabel, margin + cardW + 6, y + 18);

    // 3. Trend Dynamic
    const trendBg = trendDiff > 0.5 
      ? { fill: [240, 253, 244], stroke: [187, 247, 208], text: [22, 163, 74], label: isEn ? "Improving" : "En Hausse" } // Green
      : trendDiff < -0.5 
        ? { fill: [254, 242, 242], stroke: [254, 202, 202], text: [220, 38, 38], label: isEn ? "Declining" : "En Baisse" } // Red
        : { fill: [248, 250, 252], stroke: [226, 232, 240], text: [100, 116, 139], label: isEn ? "Stable" : "Stable" }; // Slate

    doc.setFillColor(trendBg.fill[0], trendBg.fill[1], trendBg.fill[2]);
    doc.setDrawColor(trendBg.stroke[0], trendBg.stroke[1], trendBg.stroke[2]);
    doc.rect(margin + (cardW * 2) + 6, y, cardW, cardH, 'FD');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(trendBg.text[0], trendBg.text[1], trendBg.text[2]);
    doc.text(isEn ? "PROGRESS TREND" : "DYNAMIQUE DE PROGRÈS", margin + (cardW * 2) + 9, y + 5);
    doc.setFontSize(13);
    doc.setTextColor(15, 23, 42);
    doc.text(`${trendDiff >= 0 ? '+' : ''}${trendDiff.toFixed(2)}`, margin + (cardW * 2) + 9, y + 13);
    doc.setFontSize(6.5);
    doc.setTextColor(100, 116, 139);
    doc.text(trendBg.label, margin + (cardW * 2) + 9, y + 18);

    // 4. Completed Assessments
    doc.setFillColor(239, 246, 255); // Blue 50
    doc.setDrawColor(191, 219, 254); // Blue 200
    doc.rect(margin + (cardW * 3) + 9, y, cardW, cardH, 'FD');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(37, 99, 235); // Blue 600
    doc.text(isEn ? "COMPLETED EXAMS" : "ÉVALUATIONS EFFECTUÉES", margin + (cardW * 3) + 12, y + 5);
    doc.setFontSize(13);
    doc.setTextColor(15, 23, 42);
    doc.text(`${totalTestsNum}`, margin + (cardW * 3) + 12, y + 13);
    doc.setFontSize(6.5);
    doc.setTextColor(100, 116, 139);
    const bestScoreVal = totalTestsNum > 0 ? Math.max(...grades.map(g => (g.score / g.maxScore) * 20)) : 0;
    doc.text(`${isEn ? 'Max:' : 'Max :'} ${bestScoreVal.toFixed(1)}/20`, margin + (cardW * 3) + 12, y + 18);
    ...
    // Diagnostics / Insights Box
    checkPageBreak(38);
    doc.setFillColor(255, 255, 255);
    doc.setDrawColor(226, 232, 240);
    doc.rect(margin, y, contentWidth, 34, 'FD');

    doc.setFillColor(241, 245, 249); // light blue gray title strip
    doc.rect(margin, y, contentWidth, 7, 'F');
    doc.setDrawColor(226, 232, 240);
    doc.line(margin, y + 7, margin + contentWidth, y + 7);
    ...
    doc.setFontSize(8);
    doc.setTextColor(51, 65, 85);
    doc.text(isEn ? "🔍 ACADEMIC DIAGNOSIS & RECOMMENDATIONS" : "🔍 SYNTHÈSE DU DIAGNOSTIC ACADÉMIQUE & RECOMMANDATIONS ÉDUCATIVES", margin + 4, y + 4.5);

    // Insights columns
    // Left: Strongest & Weakest
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7.5);
    doc.setTextColor(15, 23, 42);
    doc.text(isEn ? "Key Subjects Insights:" : "Analyse par Matières :", margin + 4, y + 12);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(71, 85, 105);
    if (strongest) {
      doc.text(`${isEn ? '• Main Strength:' : '• Point fort principal :'} ${strongest.subject} (Avg: ${strongest.average.toFixed(1)}/20)`, margin + 4, y + 17);
    } else {
      doc.text(`• ${isEn ? 'No strength identified' : 'Aucun point fort identifié'}`, margin + 4, y + 17);
    }

    if (weakest && strongest?.subject !== weakest.subject) {
      doc.text(`${isEn ? '• Area for Focus:' : '• Point d\'amélioration :'} ${weakest.subject} (Avg: ${weakest.average.toFixed(1)}/20)`, margin + 4, y + 22);
    } else {
      doc.text(`• ${isEn ? 'Consistent profile' : 'Profil homogène'}`, margin + 4, y + 22);
    }

    // Vertical line divider inside diagnostics
    doc.setDrawColor(226, 232, 240);
    doc.line(margin + 90, y + 9, margin + 90, y + 31);

    // Right: Personalized advice
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7.5);
    doc.setTextColor(15, 23, 42);
    doc.text(isEn ? "Personalized Educational Advice:" : "Conseil Pédagogique Personnalisé :", margin + 94, y + 12);
    ...
    if (avgBase20Val >= 14) {
      if (stdDev < 1.8) {
        advice1 = isEn 
          ? "Excellent profile with great consistency. The student performs solidly on every test."
          : "Excellent profil et travail très régulier. L'élève fait preuve d'une grande rigueur à chaque examen.";
        advice2 = isEn
          ? "Recommendation: Encourage them to participate in advanced study circles or peer-tutoring."
          : "Recommandation : Encourager l'élève à participer à des travaux d'approfondissement ou d'entraide.";
      } else {
        advice1 = isEn
          ? "Great averages, but shows slight fluctuations. Some topics are mastered better than others."
          : "Moyenne élevée mais volatile. Certaines thématiques sont maîtrisées brillamment, d'autres moins.";
        advice2 = isEn
          ? "Recommendation: Maintain focus during revisions to secure excellent results across all chapters."
          : "Recommandation : Stabiliser la rigueur lors des révisions pour uniformiser l'excellence.";
      }
    } else if (avgBase20Val >= 10) {
      if (stdDev < 1.8) {
        advice1 = isEn
          ? "Fair and highly consistent results. Work is methodical but could benefit from deeper study."
          : "Travail sérieux, régulier et convenable. L'élève assimile bien mais peut viser plus haut.";
        advice2 = isEn
          ? "Recommendation: Practice more mock exams to gain speed and reach the honors threshold."
          : "Recommandation : S'exercer sur des annales pour gagner en rapidité et franchir un palier.";
      } else {
        advice1 = isEn
          ? "Academic profile is highly inconsistent. Excellent grades are mixed with weaker ones."
          : "Profil scolaire en dents de scie. Des sommets de réussite alternent avec des notes fragiles.";
        advice2 = isEn
          ? "Recommendation: Review foundation methods and prepare worksheets to structure the learning."
          : "Recommandation : Revoir la méthodologie d'apprentissage et faire des fiches de synthèse.";
      }
    } else {
      advice1 = isEn
        ? "Fragile academic standing requiring immediate attention and focused support."
        : "Résultats d'ensemble fragiles. L'acquisition des notions fondamentales doit être surveillée.";
      advice2 = isEn
        ? "Recommendation: Implement an individual catch-up plan and contact the pedagogical team."
        : "Recommandation : Mettre en place un plan de soutien et travailler en binôme de manière ciblée.";
    }

    doc.text(advice1, margin + 94, y + 17, { maxWidth: contentWidth - 94 - 4 });
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(15, 118, 110);
    doc.text(advice2, margin + 94, y + 25, { maxWidth: contentWidth - 94 - 4 });
    ...
    doc.setFontSize(9);
    doc.setTextColor(15, 23, 42);
    doc.text(isEn ? "📊 DETAILED SUBJECT BY SUBJECT AVERAGES" : "📊 ANALYSE DÉTAILLÉE DES MOYENNES PAR DISCIPLINE", margin, y);
    y += 4;
    ...
    doc.text(isEn ? "Subject / Discipline" : "Discipline / Matière", margin + 4, y + 4.5);
    doc.text(isEn ? "Calculated Average" : "Moyenne Calculée", margin + 65, y + 4.5);
    doc.text(isEn ? "Number of Evaluations" : "Nombre d'Évaluations", margin + 105, y + 4.5);
    doc.text(isEn ? "Indicative Status / Remarks" : "Appréciation indicative", margin + 145, y + 4.5);
    ...
    if (subjectAveragesList.length === 0) {
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(8);
      doc.setTextColor(148, 163, 184);
      doc.text(isEn ? "No subjects recorded yet." : "Aucune matière enregistrée pour le moment.", margin + 4, y + 5);
      y += 8;
    } else {
      subjectAveragesList.forEach((subj, idx) => {
        checkPageBreak(8);
        if (idx % 2 === 0) {
          doc.setFillColor(248, 250, 252);
        ...
        doc.setFontSize(7.5);
        doc.setTextColor(51, 65, 85);
        doc.text(subj.subject, margin + 4, y + 4.5);

        // Average with conditional color
        if (subj.average >= 14) {
          doc.setTextColor(16, 185, 129); // Green
        } else if (subj.average >= 10) {
          doc.setTextColor(37, 99, 235); // Blue
        } else {
          doc.setTextColor(239, 68, 68); // Red
        }
        doc.text(`${subj.average.toFixed(1)} / 20`, margin + 65, y + 4.5);

        doc.setFont('helvetica', 'normal');
        doc.setTextColor(100, 116, 139);
        doc.text(`${subj.count} eval${subj.count > 1 ? 's' : ''}`, margin + 105, y + 4.5);

        // Status text
        doc.setFont('helvetica', 'italic');
        let statusText = '';
        if (subj.average >= 16) {
          statusText = isEn ? "Excellent standing" : "Excellent niveau";
        } else if (subj.average >= 14) {
          statusText = isEn ? "Solid achievements" : "Niveau satisfaisant";
        } else if (subj.average >= 12) {
          statusText = isEn ? "Honorable progress" : "Assez bon niveau";
        } else if (subj.average >= 10) {
          statusText = isEn ? "Passing grade" : "Niveau moyen, doit consolider";
        } else {
          statusText = isEn ? "Needs support" : "Lacunes à combler d'urgence";
        }
        doc.text(statusText, margin + 145, y + 4.5);
    ...
    // Chronological evaluations summary
    checkPageBreak(30);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.setTextColor(15, 23, 42);
    doc.text(isEn ? "📈 CHRONOLOGICAL HISTORY & PROGRESS PATH" : "📈 LOG CHRONOLOGIQUE DES NOTES ET PARCOURS DE PROGRESSION", margin, y);
    y += 4;
    ...
    const timelineGrades = chronologicalGrades.slice(-8); // Show up to last 8 evaluations for trend
    if (timelineGrades.length === 0) {
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(8);
      doc.setTextColor(148, 163, 184);
      doc.text(isEn ? "No grades chronological history." : "Historique chronologique indisponible.", margin + 4, y + 5);
      y += 8;
    } else {
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.setTextColor(100, 116, 139);
      doc.text(isEn ? "Showing last 8 evaluations in order of occurrence:" : "Affichage des 8 dernières évaluations dans l'ordre d'évaluation :", margin + 4, y + 4);
      y += 6;

      timelineGrades.forEach((g, idx) => {
        checkPageBreak(7.5);
        doc.setFillColor(248, 250, 252);
        ...
        doc.setFont('helvetica', 'semibold');
        doc.setTextColor(71, 85, 105);
        doc.text(`[#${idx + 1}]  ${new Date(g.date).toLocaleDateString(isEn ? 'en-US' : 'fr-FR')}`, margin + 4, y + 4);

        doc.setFont('helvetica', 'normal');
        doc.text(`${g.subject}  -  ${truncate(g.examName, 25)}`, margin + 35, y + 4);

        const score20 = (g.score / g.maxScore) * 20;
        doc.setFont('helvetica', 'bold');
        if (score20 >= 14) doc.setTextColor(16, 185, 129);
        else if (score20 >= 10) doc.setTextColor(37, 99, 235);
        else doc.setTextColor(239, 68, 68);
        doc.text(`${g.score}/${g.maxScore} (${score20.toFixed(1)}/20)`, margin + 140, y + 4);
    ...
    doc.text(isEn ? "HEAD CLASS TEACHER" : "LE PROFESSEUR PRINCIPAL", margin + 5, y);
    doc.text(isEn ? "COUNSELOR / DIR. OF STUDIES" : "CONSEILLER D'ORIENTATION / DIR.", margin + colW + 5, y);
    doc.text(isEn ? "PARENT SIGNATURE" : "SIGNATURE DES PARENTS", margin + (colW * 2) + 5, y);

    y += 4;
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(6.5);
    doc.setTextColor(148, 163, 184);
    doc.text(isEn ? "(Evaluated & Signed)" : "(Évalué et signé)", margin + 5, y);
          ...
          {activeStudent && (
            <>
              ...
              <button
                ...
                className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-md active:scale-97"
                title={language === 'fr' ? "Exporter le diagnostic et rapport de tendance en PDF" : "Export diagnostic & trend report as PDF"}
                type="button"
              >
                <TrendingUp className="h-4 w-4 text-emerald-200" /> {language === 'fr' ? 'Rapport de Tendance' : 'Trend Report'}
              </button>
            </>
          )}
                        ...
                        >
                          ...
                          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-150 space-y-2">
                            ...
                            <div className="space-y-1.5 pt-0.5">
                              ...
                              <button
                                onClick={handleExportTrendPDF}
                                className="mt-2 w-full py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-bold transition flex items-center justify-center gap-1 cursor-pointer active:scale-97 shadow-xs"
                                type="button"
                              >
                                <Download className="h-3 w-3 text-emerald-200" />
                                <span>{language === 'fr' ? 'Télécharger Rapport Tendance' : 'Download Trend Report'}</span>
                              </button>
                            </div>
                          </div>
                        </motion.div>
