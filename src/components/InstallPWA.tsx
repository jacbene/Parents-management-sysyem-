...
import { Download, Smartphone, Monitor, Share, X, CheckSquare, PlusSquare, ArrowUp, Sparkles } from 'lucide-react';
...
export default function InstallPWA() {
  ...
  const handleInstallClick = async () => {
    ...
    if (!deferredPrompt) {
      // If prompt event is not saved, we can't trigger native dialog, but we can notify
      return;
    }
  ...
  if (isInstalled) {
    return (
      <div className="hidden sm:inline-flex items-center gap-1 text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-1.5 text-[10.5px] font-bold">
        <CheckSquare className="h-3.5 w-3.5" /> App Installée (PWA)
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {/* Floating alert for Android / other mobile devices if they loaded for first time */}
      <AnimatePresence>
